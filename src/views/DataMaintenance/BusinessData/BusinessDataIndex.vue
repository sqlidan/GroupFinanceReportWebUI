<template>
  <div class="app-container BusinessData">
    ...
    <el-row>
        ...
        <el-col :span="4">
          <el-form-item label prop label-width="10px">
           <el-button type="primary" size="mini" @click="getTableList()">搜索</el-button>       
            <el-button type="primary" size="mini" @click="openAdd()" v-if="checkBtnPeimission(busdata.add.permission)">新增</el-button>
            <el-button type="primary" size="mini" @click="onBatchDelete()" :loading="bthloading" v-if="checkBtnPeimission(busdata.delete.permission)">批量删除</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form> 
        ...
        >
         ...
         <el-table-column align="center" label="操作" width="130" >
            <template slot-scope="scope" >
                <div class="tableBtn"  @click="openUpdate(scope.row)" v-if="checkBtnPeimission(busdata.update.permission)">编辑</div>
                <div class="tableBtn"  @click="deletesinge(scope.row)" v-if="checkBtnPeimission(busdata.delete.permission)">删除</div>                      
            </template>
          </el-table-column>
      ...
      ></el-pagination>
     <EditBusinessData  ref="EditComp" :pshow="EditComp.show" @on-show-change="onEditCompShowChange"
...
import { GetBusinessDataQueryList,BathDeleteBusinessData } from "api/DataMaintenance/BusinessData";
import EditBusinessData from  "./EditBusinessData"
import {warnMsg,successMsg} from 'utils/messageBox'
import {checkBtnPeimission,busdata} from 'utils/btnRole'

export default {
  name: "BusinessDataIndex",
  components: { treeSelect,
                tree,EditBusinessData},
  mixins: [tableMixin],
  data() {
    return {
      busdata,
  ...
  methods: {
    ...
    getTableList() {
       if(this.search.code==="" || this.search.code===null ||this.search.code===undefined)
      {
       ...
       this.$refs.table.clearSelection();
        GetBusinessDataQueryList(data).then(res => {
          if(res.success)
          {
    ...
    deletesinge(row){
       this.$confirm('是否删除该业务数据?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                BathDeleteBusinessData([row.id]).then(res => {
                    if(res.success)
                        this.getTableList()
                    }
                )
    ...
    onBatchDelete() {
      ...
      this.$confirm("是否确定删除", "提示", {
        ...
        BathDeleteBusinessData(arr).then(res => {
    ...
    openUpdate(row){
      ...
      this.$refs.EditComp.GetBusinessDataById(row.id);
    },
    openAdd(){
      this.EditComp.show = true
